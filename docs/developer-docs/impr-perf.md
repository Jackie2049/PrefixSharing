...
- 已完成 4090 GPU standalone microbenchmark 与全部 verl/Megatron/FSDP 端到端训练摸底（含 phase-level 归因和 batch-size scaling）；尚未完成 NPU profiler。GPU 已确认的结论可以用于当前优化决策，涉及 NPU 的结论仍需在目标环境复验。
...
- 模型：
  - FSDP 测试：Qwen3-1.7B（16Q/8KV/128D，~1.7B params，GQA 2:1，bf16 ~3.4GB）
  - Megatron 测试：Qwen3-0.6B（16Q/8KV/1024D/28L，~0.6B params，bf16 ~1.2GB）
- conda env：verl080
...
**Megatron 可行性验证（2026-07-09 补跑）**：

Qwen3-0.6B（16Q/8KV/1024D/28L, ~0.6B params, bf16 ~1.2GB）在 4090 上 Megatron 1GPU TP=1 可行：
- 模型 bf16 ~1.2GB + master fp32 ~2.4GB + DDP buffer ~2.4GB = 训练侧 ~6.8GB
- vLLM 占用剩余 ~17GB，actor HBM peak 最大约 15.9GB（bs16_p1024 PS=0），完全可容纳
- 结论：**Qwen3 系列中 0.6B 是 4090 Megatron colocate 的可行上限**，1.7B 因 DDP buffer ~6.8GB 超标不可行

**实验配置**：

实验 A（phase-level attribution）：

FSDP DP=7, n=1, 7 组 × 2 PS = 14 组
...
p=256_r32 batch scaling（数据集 64 行）：

| BS | PS | step_s | gen_s | old_logprob_s | update_actor_s | actor_HBM_GB | tok/s | 状态 |
|---:|---|---:|---:|---:|---:|---:|---:|---|
| 7 | 0 | 10.59 | 1.21 | 1.17 | 3.07 | 5.76 | 20.4 | OK |
| 7 | 1 | 10.62 | 1.23 | 1.19 | 3.08 | 5.76 | 20.3 | OK |
| 14 | 0 | 10.72 | 1.29 | 1.18 | 3.12 | 5.76 | 40.4 | OK |
| 14 | 1 | 10.67 | 1.30 | 1.15 | 3.04 | 5.76 | 40.6 | OK |
| 21 | 0 | 10.71 | 1.29 | 1.19 | 3.13 | 5.76 | 60.6 | OK |
| 21 | 1 | 10.68 | 1.31 | 1.15 | 3.04 | 5.76 | 60.7 | OK |
| 28 | 0 | 11.20 | 1.40 | 1.20 | 3.15 | 5.76 | 77.3 | OK |
| 28 | 1 | 11.00 | 1.45 | 1.21 | 3.10 | 5.76 | 78.7 | OK |
| 42 | 0 | 19.06 | 7.35 | 3.00 | 3.35 | 6.83 | 68.1 | OK |
| 42 | 1 | 20.67 | 9.09 | 2.92 | 3.28 | 5.76 | 62.8 | OK |
| 56 | 0 | 18.70 | 7.17 | 3.00 | 3.41 | 8.59 | 92.5 | OK |
| 56 | 1 | 20.74 | 9.04 | 2.98 | 3.29 | 5.76 | 83.4 | OK |
| 70 | 0/1 | - | - | - | - | - | - | DATA_EMPTY |
| 84 | 0/1 | - | - | - | - | - | - | DATA_EMPTY |

**Max batch summary (数据集限制)**：

| Engine | Prompt | Response | PS=0 max batch | PS=1 max batch | 真实 OOM 边界 | 备注 |
|---|---:|---:|---:|---:|---:|---|
| FSDP DP=7 | 512 | 64 | 56 | 56 | ≥56 | 数据集仅64行，无法验证 >56 是否 OOM |
| FSDP DP=7 | 1024 | 64 | 14 | 14 | ≥14 | 数据集仅16行 |
| FSDP DP=7 | 256 | 32 | 56 | 56 | ≥56 | 数据集64行 |
| **Megatron 1GPU** | **256** | **32** | **32** | **32** | **≥32** | 无数据集限制，bs=32 未 OOM，但 bs=48 未测 |
| **Megatron 1GPU** | **512** | **64** | **32** | **32** | **≥32** | 无数据集限制 |
| **Megatron 1GPU** | **1024** | **64** | **16** | **16** | **≥16** | bs=24 因数据集仅16行无法验证 |

**实验 B FSDP + Megatron 合并结论（2026-07-09）**：

1. **PS=ON HBM 节省随 batch 增大而显著**：

| Prompt 规格 | BS | PS=0 HBM | PS=1 HBM | HBM 节省% |
|---:|---:|---:|---:|---:|
| p=256_r32 | ≤28 | 7.62 | 7.62 | 0% |
| p=256_r32 | 42 | 6.83 | 5.76 | 15.7% |
| p=256_r32 | 56 | 8.59 | 5.76 | **33.0%** |
| p=512_r64 | ≤21 | 7.62 | 7.62 | 0% |
| p=512_r64 | 28 | 9.14 | 7.62 | 16.7% |
| p=512_r64 | 35 | 8.74 | 5.76 | 33.9% |
| p=512_r64 | 42 | 10.19 | 5.76 | 43.5% |
| p=512_r64 | 56 | 13.08 | 5.81 | **55.6%** |
| p=1024_r64 | ≤14 | 5.76-7.62 | 5.76-7.62 | 0% |

2. **PS=0 HBM 随 batch 近似线性增长**，**PS=1 HBM 在 bs≥35 后稳定在 ~5.76GB**。KV cache 是 HBM 增长的主要贡献者，PS=ON 的 prefix sharing 减少了 KV cache 重复存储。

3. **PS=ON throughput 在大 batch 时略低**（-8%），原因是 gen_s 增加。但 PS=0 的 HBM 消耗更严重，如果 batch 继续增大，PS=0 会先 OOM。

4. **数据集大小限制了 OOM 边界验证**：p=256/512 最大 batch PS=0=PS=1=56（受 64行数据限制）；p=1024 最大 batch=14（受 16行限制）。**后续应增加数据集至 128-256 行或用真实数据集验证 OOM 边界差异**。

5. **bs≥35 存在突变点**：step_s 从 ~11s 突变到 ~19-20s，gen_s 从 ~1.5s 到 ~7-9s。vLLM rollout 在大 batch 时内存管理策略切换。

---

**Megatron 补跑结果（2026-07-09 Qwen3-0.6B 1GPU TP=1 4090 实测）**：

**回填表 — 实验 A Megatron（phase-level 归因）**：

| Engine | Parallel | Model | Config | PS | gen_s | old_log_prob_s | update_actor_s | update_weights_s | step_s | tokens/s | actor_HBM_GB | conclusion |
|---|---|---|---|---|---|---|---:|---:|---:|---:|---:|---:|---:|---|
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs4_p256_r32 | 0 | 11.46 | 1.70 | 0.30 | 1.44 | 15.08 | 57.2 | 10.68 | baseline |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs4_p256_r32 | 1 | 13.20 | 1.70 | 0.45 | 1.71 | 17.25 | 50.0 | 10.68 | gen+15.2%, train≈0% |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs8_p256_r32 | 0 | 10.87 | 1.71 | 0.32 | 1.61 | 14.71 | 117.3 | 10.84 | baseline |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs8_p256_r32 | 1 | 13.29 | 1.76 | 0.60 | 1.69 | 17.54 | 98.3 | 10.68 | gen+22.2%, train≈0% |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs16_p256_r32 | 0 | 11.28 | 1.75 | 0.43 | 1.67 | 15.44 | 223.8 | **15.44** | baseline |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs16_p256_r32 | 1 | 13.32 | 1.87 | 0.95 | 1.70 | 18.16 | 190.3 | 10.68 | gen+18.1%, **HBM-31%** |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs4_p512_r64 | 0 | 11.32 | 1.70 | 0.32 | 1.61 | 15.14 | 93.1 | 10.68 | baseline |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs4_p512_r64 | 1 | 13.30 | 1.70 | 0.43 | 1.43 | 17.06 | 82.6 | 10.68 | gen+17.5%, train≈0% |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs8_p512_r64 | 0 | 11.37 | 1.75 | 0.39 | 1.62 | 15.39 | 183.3 | 13.75 | baseline |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs8_p512_r64 | 1 | 13.29 | 1.79 | 0.62 | 1.63 | 17.63 | 160.0 | 10.68 | gen+16.9%, **HBM-22%** |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs16_p512_r64 | 0 | 11.55 | 1.86 | 0.61 | 1.55 | 15.88 | 355.8 | 13.76 | baseline |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs16_p512_r64 | 1 | 13.30 | 2.02 | 1.05 | 1.57 | 18.25 | 309.5 | 10.68 | gen+15.1%, **HBM-22%** |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs4_p1024_r64 | 0 | 11.52 | 1.69 | 0.35 | 1.43 | 15.21 | 119.8 | 11.10 | baseline |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs4_p1024_r64 | 1 | 13.80 | 1.77 | 0.48 | 1.49 | 18.13 | 100.6 | 10.68 | gen+19.8%, train≈0% |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs8_p1024_r64 | 0 | 11.13 | 1.75 | 0.42 | 1.65 | 15.24 | 239.2 | **15.95** | baseline |
| megatron | 1gpu_tp1 | Qwen3-0.6B | bs8_p1024_r64 | 1 | 14.16 | 1.72 | 0.61 | 1.25 | 18.03 | 202.1 | 10.68 | gen+27.2%, **HBM-33%** |
...
**回填表 — 实验 B Megatron（batch-size scaling）**：

p=256_r32 batch scaling（Megatron 1GPU TP=1）：

| BS | PS | step_s | gen_s | old_logprob_s | update_actor_s | update_weights_s | actor_HBM_GB | tok/s | 状态 |
|---:|---|---:|---:|---:|---:|---:|---:|---:|---|
| 4 | 0 | 4.62 | 0.74 | 0.42 | 1.76 | 1.48 | 12.99 | 187.8 | OK |
| 4 | 1 | 4.46 | 0.79 | 0.33 | 1.71 | 1.43 | 12.08 | 194.4 | OK |
| 8 | 0 | 4.77 | 0.77 | 0.49 | 1.82 | 1.45 | 15.29 | 362.8 | OK |
| 8 | 1 | 4.89 | 0.80 | 0.39 | 2.12 | 1.32 | 12.77 | 353.9 | OK |
| 16 | 0 | 19.95 | 11.18 | 2.26 | 4.38 | 1.55 | **15.44** | 173.2 | OK |
| 16 | 1 | 5.42 | 0.86 | 0.65 | 2.30 | 1.24 | 14.23 | 640.9 | OK |
| 24 | 0 | 21.19 | 11.37 | 2.72 | 4.87 | 1.58 | 13.15 | 244.8 | OK |
| 24 | 1 | 6.81 | 0.90 | 1.03 | 2.97 | 1.44 | 13.62 | 763.5 | OK |
| 32 | 0 | 21.85 | 11.55 | 2.86 | 5.14 | 1.58 | **15.45** | 316.4 | OK |
| 32 | 1 | 7.51 | 0.94 | 1.20 | 3.38 | 1.42 | 14.43 | 922.4 | OK |

p=512_r64 batch scaling：

| BS | PS | step_s | gen_s | old_logprob_s | update_actor_s | actor_HBM_GB | tok/s | 状态 |
|---:|---|---:|---:|---:|---:|---:|---:|---|
| 4 | 0 | 4.95 | 1.05 | 0.44 | 1.77 | 14.45 | 285.7 | OK |
| 4 | 1 | 4.65 | 1.09 | 0.34 | 1.60 | 12.95 | 304.5 | OK |
| 8 | 0 | 18.77 | 11.65 | 1.82 | 3.77 | 13.75 | 150.3 | OK |
| 8 | 1 | 5.50 | 1.13 | 0.46 | 2.11 | 14.18 | 514.3 | OK |
| 16 | 0 | 21.39 | 11.84 | 2.67 | 4.77 | 13.76 | 264.0 | OK |
| 16 | 1 | 6.49 | 1.20 | 0.85 | 2.59 | 14.26 | 872.4 | OK |
| 24 | 0 | 22.98 | 12.20 | 3.16 | 5.44 | 13.77 | 368.9 | OK |
| 24 | 1 | 8.01 | 1.68 | 1.25 | 3.22 | 14.34 | 1059.7 | OK |
| 32 | 0 | 5.89 | 1.23 | 0.61 | 2.31 | **21.03** | 1921.8 | OK |
| 32 | 1 | 8.49 | 1.74 | 1.37 | 3.61 | 15.29 | 1331.6 | OK |

p=1024_r64 batch scaling：

| BS | PS | step_s | gen_s | old_logprob_s | actor_HBM_GB | tok/s | 状态 |
|---:|---|---:|---:|---:|---:|---:|---|
| 4 | 0 | 5.08 | 1.06 | 0.46 | 15.54 | 358.1 | OK |
| 4 | 1 | 4.68 | 1.10 | 0.34 | 13.37 | 389.0 | OK |
| 8 | 0 | 20.21 | 11.63 | 2.21 | **15.95** | 180.4 | OK |
| 8 | 1 | 4.09 | 0.90 | 0.25 | 14.82 | 890.7 | OK |
| 16 | 0 | 21.69 | 11.86 | 2.74 | **15.94** | 335.9 | OK |
| 16 | 1 | 20.60 | 12.70 | 2.02 | 10.68 | 353.6 | OK |
| 24 | 0/1 | - | - | - | - | - | OOM |

**实验 B Megatron 发现（2026-07-09）**：

1. **Megatron BS≥16 突变点比 FSDP 更早**：p=256 在 bs=16 时就出现 step_s 从 ~4.6s 突变到 ~20s（类似 FSDP bs=35 突变）。这个突变点随 prompt 长度提前：p=256 在 bs=16，p=512 在 bs=8，p=1024 在 bs=8。
2. **PS=ON 完全消除突变**：PS=1 下所有配置的 gen_s 保持在 <2s（0.74-1.74s），无突变。因为 vLLM 的 block table 在 PS=ON 下共享 prompt KV，不需要大量显存重新组织。
3. **PS=ON throughput 在大 batch 时远超 PS=0**：bs32_p256 时 PS=1 的 tok/s = 922 vs PS=0 = 316（**+192%**），因为 PS=0 在突变区 gen_s 慢了 12×。
4. **Megatron HBM 规模**：PS=0 在 bs8 时 actor HBM 可达 15.3-15.9GB，PS=1 则稳定在 10.7-14.8GB。最大节省在 bs32_p512 时 PS=0 HBM=21.03GB（接近 OOM 边界），PS=1 HBM=15.29GB（**节省 27%**）。
5. **HBM 节省汇总对比**：

| 规格 | BS | PS=0 HBM (GB) | PS=1 HBM (GB) | 节省% | 引擎 |
|---:|---:|---:|---:|---:|---|
| p=256_r32 | 16 | 15.44 | 10.68 | **30.8%** | Megatron 1GPU |
| p=256_r32 | 32 | 15.45 | 14.43 | 6.6% | Megatron 1GPU |
| p=512_r64 | 8 | 13.75 | 10.68 | **22.3%** | Megatron 1GPU |
| p=512_r64 | 32 | 21.03 | 15.29 | **27.3%** | Megatron 1GPU |
| p=1024_r64 | 8 | 15.95 | 10.68 | **33.0%** | Megatron 1GPU |
| p=1024_r64 | 16 | 15.94 | 10.68 | **33.0%** | Megatron 1GPU |

6. **PS=ON 在 Mem 和 Throughput 上都有明显正向收益**：在小 bs 时 PS=ON 仅 gen 略慢（+5-15%），但在突变区后的中等 bs 时，PS=ON 通过避免突变反转了 throughput 差距。在 0.6B 这样的小模型上，**PrefixSharing 在 Megatron colocate 场景下是纯正的正面收益**。
...
|---:|---:|---:|---:|---|
| p=256_r32 | 16 | 15.44 | 10.68 | **30.8%** |
| p=256_r32 | 32 | 15.45 | 14.43 | 6.6% |
| p=512_r64 | 32 | 21.03 | 15.29 | **27.3%** |
| p=1024_r64 | 16 | 15.94 | 10.68 | **33.0%** |

6. **PS=ON 在 Megatron 小模型场景是纯正收益**：不仅节省 HBM（最多 33%），而且在突变区后的中等 BS 下，通过避免 gen_s 爆发，throughput 反超 PS=0 最多 **192%**。

7. **差距对比——Megatron vs FSDP**：

| 观察维度 | Megatron 1GPU TP=1 (0.6B) | FSDP 7GPU DP=7 (1.7B) |
|---|---|---|
| 突变点 | BS≥16 (p=256) / BS≥8 (p=512/1024) | bs≥35 |
...
- **完整 verl/Megatron/FSDP 训练中 phase-level 归因完成**：old_log_prob, update_actor, update_weights 均已切分计时验证。
- **HBM 节省提升有效吞吐**：Megatron 场景已验证（最高 +192% throughput）。FSDP 场景因 DP 通信开销和数据集限制未能充分验证突破 OOM 边界。
