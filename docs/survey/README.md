...
| ⭐⭐⭐ | DTA (蚂蚁) | 蚂蚁/AReaL | 同 issue #6401 | [areal-project/AReaL feat/dta](https://github.com/areal-project/AReaL/tree/feat/dta) | Trie 树 DFS 序列化 + Pop/Push 栈式 KV cache + chunked backward | 同关注 RL 前缀复用、attention 层优化 | 他们走独立前缀复用调度器（1,406 行），DTA 模式下平行化走 ZeRO-1（朴素 DP），FSDP/Megatron 尚未适配 |
