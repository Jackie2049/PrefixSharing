#!/usr/bin/env bash
# Focused FSDP single-GPU end-to-end training profiler
# Same config matrix as Megatron for fair comparison
set -xeuo pipefail

SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
RESULTS_DIR="${SCRIPT_DIR}/../perf_results/fsdp_single_gpu"
mkdir -p "$RESULTS_DIR"

PREFIX=/home/zxw/miniconda3/envs/verl080_prefix-sharing
MODEL_PATH=/home/zxw/.cache/huggingface/hub/models--Qwen--Qwen2.5-0.5B/snapshots/060db6499f32faf8b98477b0a26969ef7d8b9987
DATA_PATH=/home/zxw/verldir_clean/data

cd /home/zxw/verldir_clean
PS_PYTHONPATH=/home/zxw/prefix-sharing

# Same key configs as Megatron route
CONFIGS=(
  "2|64|32|0.6|8"
  "4|64|32|0.6|16"
  "8|64|32|0.6|32"
...
for cfg in "${CONFIGS[@]}"; do
  ...
  for ps in 0 1; do
    COUNT=$((COUNT + 1))
    echo "========================================"
    echo "[$COUNT/$TOTAL] FSDP: bs=${bs} prompt=${pl} response=${rl} PS=${ps} gmem=${gmem}"
    echo "========================================"
    ...
    LOGFILE="${RESULTS_DIR}/fsdp_bs${bs}_p${pl}_r${rl}_ps${ps}.log"

    timeout 300 \
      PYTHONPATH=$PS_PYTHONPATH${PYTHONPATH:+:$PYTHONPATH} \
      $PREFIX/bin/python3 -m verl.trainer.main_ppo \
        ...
        trainer.experiment_name=fsdp_bs${bs}_p${pl}_r${rl}_ps${ps} \
...
echo "=== FSDP single GPU profile done ==="
